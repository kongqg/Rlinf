...
def _build_openpi_model_for_fsdp(
    ...
    has_full_weights = os.path.exists(full_weights_path) or os.path.exists(
        actor_full_weights_path
    )
    if args.is_lora and args.lora_path is None and has_full_weights:
        from rlinf.models.embodiment.openpi.lora_utils import apply_openpi_lora

        _log_stage(
            f"applying LoRA architecture before loading LoRA checkpoint rank={args.lora_rank}",
            dist_ctx,
            enabled=args.debug_stage_logs,
            all_ranks=args.debug_all_ranks,
        )
        model = apply_openpi_lora(model, lora_rank=args.lora_rank, lora_path=None)
