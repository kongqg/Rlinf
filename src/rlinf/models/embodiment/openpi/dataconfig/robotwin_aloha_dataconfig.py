...
class LeRobotAlohaDataConfig(DataConfigFactory):
    ...
    # If set, overrides the default normalization mode chosen by the upstream
    # OpenPI config factory. This is useful when we want to bootstrap a new
    # task with z-score normalization before deciding whether quantile-based
    # normalization is worth keeping.
    ...
    def create(
        ...
        if self.use_quantile_norm is not None:
            base_config = dataclasses.replace(
                base_config, use_quantile_norm=self.use_quantile_norm
            )
