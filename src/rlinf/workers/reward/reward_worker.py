...
class RewardWorker(Worker):
    """Reward worker stub kept only for compatibility in the minimal vendor."""
    ...
    def init_worker(self):
        ...
        raise NotImplementedError(
            "Reasoning/agent reward workers were not kept in the minimal local RLinf "
            "vendor. This workspace only keeps the embodied reward path."
        )
...
class EmbodiedRewardWorker(Worker):
    ...
    def model_provider_func(self):
        raise NotImplementedError(
            "Embodied reward models were not kept in the minimal local RLinf vendor. "
            "Set reward.use_reward_model=False for the baseline."
        )
...
class FSDPRewardWorker(FSDPModelManager, Worker):
    ...
    def model_provider_func(self):
        raise NotImplementedError(
            "Reward-model training was not kept in the minimal local RLinf vendor."
        )
    ...
    def build_dataloader(self) -> tuple[Optional[DataLoader], Optional[DataLoader]]:
        raise NotImplementedError(
            "Reward-model dataset training was not kept in the minimal local RLinf "
            "vendor."
        )
