...
class _VisualTaskConfig(dict):
    """Keep visualization-related fields alive across RoboTwin's eval defaults."""

    def __init__(
        self,
        payload: dict,
        *,
        render_freq: Optional[int] = None,
        eval_video_log: Optional[bool] = None,
    ):
        super().__init__(payload)
        self._render_freq = render_freq
        self._eval_video_log = eval_video_log

    def __setitem__(self, key, value):
        if key == "render_freq" and self._render_freq is not None:
            value = self._render_freq
        elif key == "eval_video_log" and self._eval_video_log is not None:
            value = self._eval_video_log
...
class RoboTwinEnv(gym.Env):
    ...
    def _init_env(self):
        ...
        import envs.robot.planner as planner_module
        import envs.robot.robot as robot_module

        if getattr(planner_module, "CuroboPlanner", None) is None:
            class _MissingCuroboPlanner:
                pass

            planner_module.CuroboPlanner = _MissingCuroboPlanner
            robot_module.CuroboPlanner = _MissingCuroboPlanner
        ...
        if self.cfg.get("visualize", False):
            task_config = _VisualTaskConfig(
                task_config,
                render_freq=int(task_config.get("render_freq", 10)),
                eval_video_log=bool(task_config.get("eval_video_log", False)),
            )
    ...
    def _cal_chunk_rewards(self, step_reward, chunk_step, terminations, infos):
        ...
        )  # infos.get("n_steps_to_run", np.array([[0] for _ in range(self.num_envs)]))
