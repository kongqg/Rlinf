...
def crop_and_resize(image, crop_scale, batch_size):
    ...
    if image.ndim not in (3, 4):
        raise ValueError(f"Expected image with 3 or 4 dims, got shape {image.shape}")

    expanded_dims = image.ndim == 3
    if expanded_dims:
        image = image[None, ...]

    crop_ratio = float(np.clip(np.sqrt(crop_scale), 0.0, 1.0))
    if batch_size != image.shape[0]:
        raise ValueError(
            f"batch_size ({batch_size}) does not match image batch ({image.shape[0]})"
        )
    ...
    for img in image:
        ...
        if np.issubdtype(cropped.dtype, np.floating):
            pil_input = np.clip(cropped * 255.0, 0, 255).astype(np.uint8)
        else:
            pil_input = cropped.astype(np.uint8, copy=False)

        resized = Image.fromarray(pil_input).resize((224, 224), Image.BILINEAR)
        resized_images.append(np.asarray(resized, dtype=np.float32) / 255.0)

    output = np.stack(resized_images, axis=0)
    if expanded_dims:
        output = output[0]
    return output
...
def center_crop_image(image):
    ...
    cropped = crop_and_resize(image, crop_scale, batch_size)
    cropped = np.clip(cropped, 0.0, 1.0)

    if np.issubdtype(orig_dtype, np.integer):
        dtype_info = np.iinfo(orig_dtype)
        cropped = np.rint(cropped * dtype_info.max).astype(orig_dtype)
    else:
        cropped = cropped.astype(orig_dtype, copy=False)
